...
function ExplicatifPage({ title, icon, avantages, composants, color }) {
  ...
  return (
    <div style={{ padding: "28px 20px", maxWidth: 860, margin: "0 auto" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 10, marginBottom: 28 }}>
        {avantages.map((a, i) => (
          <div key={i} style={{ ...card, padding: 16, display: "flex", alignItems: "center", gap: 12, animation: `fadeIn 0.5s ease ${i * 0.1}s both` }}>
            <div style={{ width: 44, height: 44, background: `${color}12`, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, flexShrink: 0 }}>{a.icon}</div>
            <div style={{ fontFamily: FONT, fontSize: 14, color: C.text, fontWeight: 600 }}>{a.text}</div>
          </div>
        ))}
      </div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg }}>
      ...
      <main>
        ...
        {active === "demo-ite" && <ExplicatifPage title="Isolation Thermique par l'Extérieur" icon="🧱" avantages={ITE_AVANTAGES} composants={ITE_COMPOSANTS} color={C.bleu} />}
        {active === "demo-pv" && <ExplicatifPage title="Panneaux Solaires Photovoltaïques" icon="☀️" avantages={PV_AVANTAGES} composants={PV_COMPOSANTS} color={C.success} />}
